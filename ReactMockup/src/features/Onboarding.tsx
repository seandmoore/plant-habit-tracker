...
import { Button, CompanionRing, space, useTheme } from '@/ui';
...
const pages = [
  ...
  },
  {
    ...
    message: 'Explore a starter catalog, try photo suggestions, and ask the companion to explain saved care information.',
  },
] as const;
...
export function Onboarding() {
  ...
  return (
    <LinearGradient
      ...
      style={[styles.root, { paddingTop: insets.top + 30, paddingBottom: insets.bottom + 30 }]}
    >
      <View style={styles.content}>
        <CompanionRing state={isLastStep ? 'speaking' : 'idle'} size={82} />
        ...
        <Button
          label={isLastStep ? 'Explore my sample plants' : 'Continue'}
          onPress={advance}
          style={styles.button}
        />
        {step > 0 ? <Button label="Back" kind="plain" onPress={() => setStep(step - 1)} /> : null}
      </View>
    </LinearGradient>
  );
}
...
const styles = StyleSheet.create({
  ...
  content: { width: '100%', maxWidth: 620, alignItems: 'center', gap: space.xl - 4 },
  ...
  dots: { flexDirection: 'row', gap: 7, marginTop: space.sm + 2 },
  dot: { height: 8, borderRadius: 999 },
  button: { minWidth: 230, marginTop: space.sm },
});
