...
/// A deterministic companion used wherever no on-device model is available. Each branch answers
/// from the facts it was handed and refuses to act as a botanical database.
struct ScriptedCompanionService: CompanionService {
...
#if canImport(FoundationModels)
/// Apple's on-device model, constrained to the facts in the prompt. The instructions are the
/// guardrail: the model explains saved records and says when they are not enough, rather than
/// inventing plant facts or diagnosing disease.
@available(iOS 26.0, macOS 26.0, *)
struct FoundationModelCompanionService: CompanionService {
